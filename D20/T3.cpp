...
ll to[MAXN],nxt[MAXN],head[MAXN],cnt;
void add(ll u,ll v){
...
void dfs(ll x){
    ...
    ll inv=qpow(son_num[x],mod-2,mod);
    for(ll i=head[x];i;i=nxt[i]){//枚举重儿子
        for(ll j=0;j<=n;j++) g[j]=1;
        ll h_son=to[i];//heavy son
        for(ll j=head[x];j;j=nxt[j]){//枚举其他儿子
        ...
        for(ll j=0;j<=size[x];j++) f[x][j]=(f[x][j]+g[j]*inv%mod)%mod;
    }
    if(!head[x]) f[x][0]=1;
