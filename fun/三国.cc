		...
		if ((x >= 0) && (x < (*w).shoupaishu) && (y >= 0) && (y < (*w).shoupaishu - 1)) {
			...
			for (i = 0; i <= (((*w).shoupaishu) + 1); i++) {
				if ((*w).shoupai[i].leixing == -1) {
					for (j = i + 1; j <= (((*w).shoupaishu) + 2); j++) (*w).shoupai[j - 1] = (*w).shoupai[j];
					i--;
				}
			}
			printf("玩家把：\n");
...
pai Panpai(wujiang*w1, int*sha, int y, pai qipaidui[104], int*qipaishu) {
	...
	if (y == -1)return q;
		...
		switch ((*w1).shoupai[y].leixing) {
			...
			case 102:
				if ((*w1).juese) printf("当前不需要响应任何操作，你不能主动打出【闪】！\n");
				return p;
				break;
			case 103:
				if (((*w1).tili) < ((*w1).tilishangxian)) {
					...
					if ((*w1).juese) printf("你并未受伤，不能对自己使用【桃】！\n");
				}
				return p;
				break;
			case 210:
				if ((*w1).juese) printf("当前不需要响应任何锦囊，你不能主动打出【无懈可击】！\n");
				return p;
				break;
			case 201:
			case 202:
...
int Mubiao(pai p, wujiang *w1, wujiang *w2, int*sha) {
	...
	switch (p.leixing) {
		...
		case 251:
			if ((*w1).juese) {
				...
				if (x == 0) {
					int i;
					for (i = 0; i <= 2; i++) {
						if ((*w2).pandingpai[i].leixing == 251) i = -1;
						break;
					}
					if (i == -1) {
		...
		case 252:
			int i;
			for (i = 0; i <= 2; i++) {
				if ((*w1).pandingpai[i].leixing == 252) i = -1;
				break;
			}
			if (i == -1) {
...
void Da1pai(wujiang *w1, wujiang*w2, pai qipaidui[104], int *qipaishu, int x) {
	int i;
	if ((x < 0 || x >= ((*w1).shoupaishu)) && x != 100) printf("你的牌呢？！\n");
	else {
...
void Miaoshu(pai p, int juese, int duixiang) {
	...
	} else if (p.leixing >= 301 && p.leixing <= 400) printf("装备");
